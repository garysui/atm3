...
(`../atm2`, github `garysui/atm2`) with better data modeling; atm2 remains the
reference for proven behavior (identity resolution, adjustment math, parity
tests).
...
1. **raw** — vendor responses saved verbatim under `data/raw/`, append-only,
   cataloged in the `raw` DuckDB schema. Never edit, re-encode, or "fix" raw
   files. Raw is the only source of truth.
...
- Keep schema (migrations) and code in git. Keep local market data out of git;
...
- Keep DuckDB schema in numbered migrations under `db/migrations/`.
